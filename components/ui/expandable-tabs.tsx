...
export function ExpandableTabs({ tabs, className, activeColor = 'text-primary', onChange }: ExpandableTabsProps) {
	...
	return (
		<div className={cn('flex flex-wrap items-center gap-2 rounded-full border border-zinc-200 w-fit bg-zinc-100 p-1 shadow-sm', className)}>
			{tabs.map((tab, index) => {
				...
				return (
					<motion.button
						...
						className={cn(
							'relative flex items-center rounded-full px-2 py-1 font-medium transition-colors duration-300',
					...
					>
						<Icon
							...
							className={cn(activeTab === index ? 'text-[crimson]' : null)}
						/>
						<AnimatePresence initial={false}>
							{activeTab === index && (
								<motion.span
									...
									exit='exit'
									transition={transition}
									className={cn('overflow-hidden', activeTab === index ? 'text-[crimson]' : null)}
								>
									<p className='font-hbold'>{tab.title}</p>
								</motion.span>
							)}
						</AnimatePresence>
					</motion.button>
